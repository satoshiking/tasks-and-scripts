...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 2: 
        ...
        for id in id_list:
            r = requests.get("https://api.tport.online/v2/public-stations/" + str(id))
            d = json.loads(r.text)
            ...
            if not d.get("detail"):
                p = Postamat(
                    ...
                    accept_payments=d["accept_payments"], working_hours=d["working_hours"])

                print(" ID={:<5} STATUS_CODE={:< 5} WORKING_NOW={}".format(p.id, p.status_code, p.is_working()))
            else:
